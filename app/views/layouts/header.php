    ...
    <section class="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
        ...
        <nav class="flex flex-wrap items-center gap-2 sm:gap-3">
            <?php if (isset($_SESSION['id_utilisateur'])): ?>
                <?php
                // CORRIGÉ : les anciennes routes 'etudiant/accueil' et 'etablissement/accueil'
                // n'existent pas dans routes.php. Les bonnes routes sont 'student/home'
                // et 'institution/home' (convention anglaise adoptée lors du refactoring MVC).
                ...
                ?>
                <a href="<?= $dashboard ?>"
                   class="rounded-lg border border-white/25 px-4 py-2 text-sm font-semibold text-white hover:border-[#f1b456] hover:text-[#f1b456]">
                    Mon espace
                </a>
...
<?php
// Affichage du message flash (succès / erreur / info)
// get_flash() lit le message depuis $_SESSION['flash'] et le supprime immédiatement
...
?>
    <div class="<?= $class ?> border-b border-white/20 px-4 py-3 text-center text-sm font-semibold"
         role="alert">
