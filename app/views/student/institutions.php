        ...
        <div class="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">
                    Listes des filières disponibles.
                </p>
            </div>

            <div class="flex flex-wrap items-center justify-end gap-2 sm:gap-3">
                <a href="index.php?route=student/home"
        ...
        <form method="GET" action="index.php" class="mt-6">
            ...
            <div class="flex flex-col gap-3 sm:flex-row">
                ...
                <?php if (!empty($recherche)): ?>
                    <a
                        href="index.php?route=student/institutions"
                        class="rounded-lg border border-slate-300 px-4 py-3 text-sm font-semibold text-slate-600 hover:border-[#f1b456]">
                        Effacer
                    </a>
                <?php endif; ?>
        ...
        <?php else: ?>
            ...
            <div class="mt-6 grid gap-4 md:grid-cols-2">
                ...
                <?php foreach ($offres as $offre): ?>
                    ...
                    <a
                        href="index.php?route=student/offer/details&id=<?= (int) $offre['id_offre_filiere'] ?>"
                        class="block rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition
                               hover:-translate-y-0.5 hover:border-[#f1b456]/60 hover:shadow-md
                               focus:outline-none focus:ring-2 focus:ring-[#f1b456]">
                        ...
                        <article class="flex h-full flex-col gap-3">
                            ...
                            <!-- Badges de conditions d'accès -->
                            <?php if ($offre['serie_bac'] || $offre['moyenne_bac']): ?>

                                <div class="flex flex-wrap gap-2 text-xs">

                                    <?php if ($offre['serie_bac']): ?>
                                        <span class="rounded-lg border border-sky-200 bg-sky-50 px-3 py-1 text-sky-800">
                                            Série <?= htmlspecialchars($offre['serie_bac']) ?>
                                        </span>
                                    <?php endif; ?>

                                    <?php if ($offre['moyenne_bac']): ?>
                                        <span class="rounded-lg border border-amber-200 bg-amber-50 px-3 py-1 text-amber-800">
