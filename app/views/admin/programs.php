...
// URL d'action du formulaire selon le mode
...
<main class="mx-auto w-full max-w-6xl flex-1 px-4 py-10 sm:px-6 lg:px-8">
    <section class="rounded-3xl border border-white/20 bg-white/95 p-6 shadow-2xl shadow-[#071d3b]/25 sm:p-8">
        ...
        <div class="mt-8 grid gap-8 lg:grid-cols-2">
            ...
            <!-- ── LISTE DES FILIÈRES ── -->
            <div>
                <h2 class="text-lg font-bold text-[#071d3b]">Filières existantes</h2>
                        ...
                        <?php foreach ($filieres as $f): ?>
                            ...
                            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-[#f1b456]/60">
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    <!-- Nom + description + compteur d'offres -->
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <!-- Nombre d'offres publiées sur cette filière -->
                                    ...
                                    <div class="flex shrink-0 flex-wrap gap-2">
                                        ...
                                        <!-- Lien vers le formulaire de modification (GET ?edit=) -->
                                        <a href="index.php?route=admin/programs&edit=<?= $f['id_filiere'] ?>"
            ...
            <div>
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-6">
                    ...
                    <form method="POST" action="<?= $actionUrl ?>" novalidate class="mt-4 space-y-4">
                        ...
                        <!-- Champ caché en mode modification : identifie la filière à modifier -->
                        <?php if ($modeEdition): ?>
                            <input type="hidden"
                                   name="id_filiere"
                                   value="<?= $modeEdition['id_filiere'] ?>">
                        <?php endif; ?>
                        ...
                        <div>
                            <label for="description"
                                   class="mb-2 block text-sm font-semibold text-[#071d3b]">
                                Description
                                <span class="font-normal text-slate-400">(optionnelle)</span>
                            </label>
                            <textarea id="description"
                                      name="description"
                                      rows="4"
                                      placeholder="Courte description de la filière..."
                                      class="w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm outline-none focus:border-[#f1b456] focus:ring-2 focus:ring-[#f1b456]/30"><?= $formDescription ?></textarea>
                        </div>

                        <div class="flex gap-2">
                            <button type="submit"
                                    class="flex-1 rounded-lg bg-[#f1b456] px-5 py-3 text-sm font-bold text-[#071d3b] hover:bg-[#e4a744]">
                                <?= $modeEdition ? 'Enregistrer' : 'Ajouter au catalogue' ?>
                            </button>

                            <!-- Bouton "Annuler" uniquement en mode modification -->
                            <?php if ($modeEdition): ?>
